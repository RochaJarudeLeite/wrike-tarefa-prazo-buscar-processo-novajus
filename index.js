...
export async function handler(event) {
  ...
  for (const litigation of litigations) {
    ...
    if (litigation.groups.cnj != null) {
      ...
      try {
        ...
        cl.isValid = false;
        cl.htmlDescription.push(
          `<li><strong>❗ ${cl.litigation} - CNJ inválido.</strong></li>`
        );
  ...
  citedLitigations = citedLitigations.map((cl) => {
    let validCl = validCitedLitigations.find(
      (validCl) => validCl.litigation === cl.litigation
    );
    if (validCl != null) {
      return validCl;
    } else {
  ...
  for (const citedLitigation of citedLitigations) {
    if (
      citedLitigation.comments.length > 0 &&
      citedLitigation.errors.length > 0
    ) {
  ...
  let newTaskDescription = wrikeTask.description.replace(
    regexDescriptionInfo,
    `Processos Relacionados<\/b><br \/><ul>${newDescriptionInfo}<\/ul>`
  );
